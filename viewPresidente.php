...
<body class="bg-light">
  <div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
      ...
      <div>
        <span class="me-3">Bienvenido <strong><?= $_SESSION['user'] ?></strong>
    ...
    <?php if ($mensaje): ?>
    <div id="mensajeCuota" class="alert alert-info"><?= $mensaje ?></div>
    <script>
    ...
    <table class="table table-bordered table-striped">
      ...
      <tbody>
        <?php foreach ($vecinos as $vecino):
                    ...
                    ?>
        <tr>
          <td><?= htmlspecialchars($vecino['nombre']) ?></td>
          <td><?= htmlspecialchars($vecino['apellidos']) ?></td>
          <td><?= htmlspecialchars($vecino['dni']) ?></td>
          <td><?= htmlspecialchars($vecino['telefono']) ?></td>
          <td><?= htmlspecialchars($vecino['email']) ?></td>
          <td><?= $vivienda ? htmlspecialchars($vivienda['piso']) : '-' ?></td>
          <td><?= $vivienda ? htmlspecialchars($vivienda['bloque']) : '-' ?></td>
          <td><?= $vivienda ? htmlspecialchars($vivienda['letra']) : '-' ?></td>
          <td>
            <?php if ($cuota): ?>
            <form method="post" class="d-flex gap-2 align-items-center">
              <input type="hidden" name="idCuota" value="<?= $cuota['id'] ?>">
              <input type="number" name="cuotasPagadas" value="<?= $cuota['cuotasPagadas'] ?>" min="0"
                class="form-control form-control-sm" style="width:80px;">
          </td>
          <td>
            <input type="number" name="cuotasImpagadas" value="<?= $cuota['cuotasImpagadas'] ?>" min="0"
              class="form-control form-control-sm" style="width:80px;">
          </td>
          <td>
            <input type="date" name="fechaUltimaCuota" value="<?= $cuota['fechaUltimaCuota'] ?>"
